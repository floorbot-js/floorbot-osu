...
const cachePath = process.env.OSU_CACHE_PATH || `${__dirname}/../cache`;

class OsuUtils {

    // This only gets key:value pairs at the moment.
    static processFile(beatmap_id) {
        const path = `${cachePath}/${beatmap_id}.osu`;
    ...
    static getBeatmaps(request, download = false) {
        return OsuApi.getBeatmaps(request).then(beatmaps => {
            ...
            return Promise.all(beatmaps.map(beatmap => {
                beatmap.path = `${cachePath}/${beatmap.beatmap_id}.osu`;
    ...
    static simulate(score, options) {
        return new Promise((resolve, reject) => {
            const path = score.path || `${cachePath}/${score.beatmap_id}.osu`;
    ...
    static difficulty(beatmap, mods) {
        ...
        return new Promise((resolve, reject) => {
            const path = beatmap.path || `${cachePath}/${beatmap.beatmap_id}.osu`;
            if (fs.existsSync(path)) return resolve(path);
            ...
            return OsuTools.difficulty(path, options).then(difficulty => {
                ...
                if (options.some(option => ['ez', 'ht', 'hr', 'dt', 'nv'].includes(option))) {
                    ...
                    diffBeatmap.diff_approach = +Number(difficulty.ar).toFixed(2);
                    diffBeatmap.diff_overall = +Number(difficulty.od).toFixed(2);
                }
                ...
                diffBeatmap.difficultyrating = +Number(diffBeatmap.difficultyrating).toFixed(2);
                diffBeatmap.diff_drain = +Number(diffBeatmap.diff_drain).toFixed(2);
                diffBeatmap.diff_size = +Number(diffBeatmap.diff_size).toFixed(2);
                diffBeatmap.bpm = +Number(diffBeatmap.bpm).toFixed(2);
